...
class Personnage(Entity):
    def __init__(self, **kwargs):
        ...
        self.collider = BoxCollider(
            self, center=Vec3(0, 2, 0), size=Vec3(2, 4, 1))
        self.spawnpoint = (0, 1, 0)
        self.vie = 3
        self.vivant = True
        self.lock_move = False
        self.all_entity = kwargs.get("all_entity")
        ...
        self.walk_speed = 10
        self.walking = False
        # the walk diection is stored here. -1 for left and 1 for right.
        self.velocity = 0
        self.jump_height = 4
        ...
        # we need to store this so we can interrupt the fall call if we try to double jump.
        self._start_fall_sequence = None

        ray = boxcast(self.world_position, self.down, distance=10, ignore=(
    ...
    def update(self):
        ...
        ).hit == False and (not self.lock_move):

            self.x += self.velocity * time.dt * self.walk_speed

        self.walking = held_keys['q'] + held_keys['d'] > 0 and self.grounded
        ...
        if self.jumping and (not self.lock_move):
        ...
        if self.vie == 0:
            self.mort_permanante(self.all_entity)
    ...
    def input(self, key):
        ...
        if held_keys['d'] or held_keys['q']:
            self.scale_x = self._original_scale_x * self.velocity

        if key == 'f' and not self.lock_move:
            for _ in range(40):
                ray = boxcast(
                    self.position+Vec3(self.velocity * time.dt *
                                       self.walk_speed, self.scale_y/2, 0),
    ...
    def jump(self):
        ...
        if boxcast(self.position+(0, .1, 0), self.up, distance=self.scale_y, thickness=.95, ignore=(self,), traverse_target=self.traverse_target).hit or self.lock_move:
            return

        if hasattr(self, 'y_animator'):
            self.y_animator.kill()
        self.jump_dust = Entity(model=Circle(), scale=.5,
                                color=color.white33, position=self.position)
        self.jump_dust.animate_scale(3, duration=.3, curve=linear)
        ...
        self.animate_y(target_y, duration, resolution=30, curve=out_expo)
    ...
    def vivre_mourir(self):
        if self.vivant == False or self.y <= -150:
            self.vie -= 1
            self.vivant = True

    def mort_permanante(self, liste):
        self.lock_move = True
        for i in liste:
            i.visible = False
...
if __name__ == '__main__':
    ...
    ground = Entity(model='cube', color=color.white,
                    origin_y=.5, scale=(20, 10, 1), collider='box')
    wall = Entity(model='cube', color=color.white33,
    ...
    def input(key):
        if held_keys['c']:
            if wall.collision == True:
                wall.collider = False

    player_controller = Personnage(scale_y=2, jump_height=4, x=3, max_jumps=3)
    camera.add_script(SmoothFollow(
        target=player_controller, offset=[0, 1, -30], speed=4))

    player_controller.position = (0, 5, 0)
